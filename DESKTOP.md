...
対象は Apple Silicon Mac。Sky Copy 配色（Zenは色指定なし）と、yabai／skhd の操作設定を共通化しています。
...
- **Zen:** ワークスペースの色指定を解除します。現在の透過表示はTransparent Zen Mod／Zen Internetの設定を併用しています。これらのMod・拡張機能の導入と設定はNixの自動同期対象に含みません。
...
この構成を作成したMacでは、Zenの13ワークスペースは色指定なし、OrcaはSky Copyに揃えています。差分がなければプロファイルを書き換えません。
...
Zen／Orcaの適用処理は合成データによる21件のテストで、テーマ以外の保持、再実行、起動中の延期、競合拒否、バックアップと復元、重複実行の排他を確認しています。
